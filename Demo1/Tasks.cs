...
namespace Demo1
{
    public class Tasks
    {
        ...
        public static int ExtraTask1Function(int input)
        {
            var output = 0;
            var num = input;
            if (input < 0)
            ...
            int min = listOfDigits[listOfDigits.Count - 1];
            int minIndex = listOfDigits.Count - 1;

            for (int i = listOfDigits.Count - 2; i >= 0; --i)
            {
                if (listOfDigits[i + 1] < min)
                {
                    min = listOfDigits[i + 1];
                    minIndex = i + 1;
                }
                if (listOfDigits[i] < listOfDigits[i + 1])
                {
                    int tmp = listOfDigits[i];
                    listOfDigits[i] = listOfDigits[minIndex];
                    listOfDigits[minIndex] = tmp;
                    break;
                }
            }
            ...
            {
                output += (int)Math.Pow(10, listOfDigits.Count - 1 - j) * listOfDigits[j];
            }
            if (input == output)
                return -1;
            return output;
